/* 
 ...
 This example sketch was modified by Bruce Knipe and renamed to EDB_ReadDB.ino.

 EDB_ReadDB.ino:
 
 Step 1) Open EDB_SD.db and using the header determine the maximum size of the table,
         the length of the records and how many records are in the table.
 Step 2) Iterate through the records displaying each one along with its ID# (which
         is also the record#) to the Serial window using Serial.print[ln]().
 Step 3) Close the file and exit.

 Basically this sketch reads from an EDB table, EDB_SD.db, and displays the contents
 to the Serial window. EDB_ReadDB.ino is part of a group of sketches that read and
 write to the EDB tables and burn the contents onto the ATmeaga328p's internal EEPROM
 flash memory on Arduino UNO boards and the AT24Cxxx modules, as well as the SD card
 being used by this sketch and associated Python scripts.

 EDB_ReadDB.ino is part of a group of Arduino sketches, Python scripts, EDB
...
Note: There are no references on where to find the Arduino, Wire, SD or EEPROM
 ...
 Modifications to EDB_ReadDB.ino are as follows:

 Most of what follows is only relevant if you comment out the #define EDB_ReadSD above.
 Otherwise this is a read only sketch and doesn't do much besides reading and displaying
 the data in the EDB_SD.db table.
  ...
  Added code to open the EDB_SD.db database table if EDB_ReadDB is #defined.
  ...
  Note: when EDB_ReadDB is defined the program becomes a read only sketch and no longer
        creates the EDB_SD.DB database.
...
// Pin connections for the SD card.
// SD pins
// MISO  digital pin 12
// SCK   digital pin 13
// MOSI  digital pin 11
...
// External EEPROM pins for a AT24Cxxx module connected to any of these related sketches.
// SCL analog pin 5
// SDA analog pin 4

File dbFile;
// Comment out #define EDB_ReadDB to recreate the database.
#define EDB_ReadDB // Just read then display the database files.
